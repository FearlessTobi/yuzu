...
[[nodiscard]] QPixmap GetAvatar(const httplib::Result& response) {
    ...
    if (it == response->headers.end()) {
        throw std::runtime_error{"'Location' header missing in response"};
    }
    QImage image = DownloadImageUrl(it->second);
    if (image.isNull()) {
        return QIcon::fromTheme(QStringLiteral("portrait_sync_error")).pixmap(48);
    } else {
        return QPixmap::fromImage(image).scaled(48, 48, Qt::IgnoreAspectRatio,
