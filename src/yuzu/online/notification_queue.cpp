...
NotificationQueue::NotificationQueue(Core::OnlineInitiator& online_initiator_,
    ...
    websocket->setOnMessageCallback([this](const ix::WebSocketMessagePtr& msg) {
        ...
        case ix::WebSocketMessageType::Error:
            ...
            if (const std::optional headers = AuthorizationHeaders(online_initiator); headers) {
...
void NotificationQueue::ParseNotification(const std::string& input) try {
    ...
    const auto display_type = static_cast<DisplayType>(json.at("display_type").get<int>());
