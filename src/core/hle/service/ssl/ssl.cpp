...
[[nodiscard]] bool IsReady(Sockets::BSD::FileDescriptor* descriptor, u16 event) {
    ...
    ASSERT(pollfd.revents == event);
    return true;
}

class ISslConnection final : public ServiceFramework<ISslConnection> {
public:
    explicit ISslConnection(std::shared_ptr<Sockets::BSD> bsd_u_, u32* num_connections_,
                            ::SSL* ssl_connection_)
        : ServiceFramework("ISslConnection"), bsd_u{bsd_u_}, num_connections{num_connections_},
          ssl_connection{ssl_connection_} {
        // clang-format off
...
private:
    ...
    void SetHostName(Kernel::HLERequestContext& ctx) {
        ...
        [[maybe_unused]] const int err = SSL_set_tlsext_host_name(ssl_connection, hostname.c_str());
        ASSERT(err == 1);
    ...
    void Read(Kernel::HLERequestContext& ctx) {
        ...
        if (!IsReady(descriptor, Network::POLL_IN)) {
    ...
    void Write(Kernel::HLERequestContext& ctx) {
        ...
        if (!IsReady(descriptor, Network::POLL_OUT)) {
            LOG_DEBUG(Service_SSL, "Would block emitted");
            IPC::ResponseBuilder rb{ctx, 3};
            rb.Push(RESULT_WOULDBLOCK);
            rb.Push<s32>(-1);
...
public:
    explicit ISslContext(std::shared_ptr<Sockets::BSD> bsd_u_, SSL_CTX* ssl_context_)
        : ServiceFramework("ISslContext"), bsd_u{std::move(bsd_u_)}, ssl_context{ssl_context_} {
        static const FunctionInfo functions[] = {
...
private:
    ...
    void CreateConnection(Kernel::HLERequestContext& ctx) {
        ...
        rb.Push(RESULT_SUCCESS);
        rb.PushIpcInterface<ISslConnection>(bsd_u, &num_connections, ssl_connection);
    }
...
public:
    explicit SSL(Core::System& system_, std::shared_ptr<Sockets::BSD> bsd_u_)
        : ServiceFramework{"ssl"}, system{system_}, bsd_u{std::move(bsd_u_)} {
...
private:
    void CreateContext(Kernel::HLERequestContext& ctx) {
        ...
        IPC::ResponseBuilder rb{ctx, 2, 0, 1};
        rb.Push(RESULT_SUCCESS);
        rb.PushIpcInterface<ISslContext>(bsd_u, ssl_context);
    }
