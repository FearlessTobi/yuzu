...
std::vector<u8> Pop(u8*& pos, size_t len) {
...
void SerializeAddrInfo(const Network::AddrInfo& input, std::vector<u8>& result) {
    ...
    addr.portno = addr.portno >> 8 | addr.portno << 8;
...
void SFDNSRES::GetHostByNameRequest(Kernel::HLERequestContext& ctx) {
    ...
    ExecuteWork(ctx, "SFDNSRES:GetHostByNameRequest", std::move(work));
}
...
void SFDNSRES::GetAddrInfoRequest(Kernel::HLERequestContext& ctx) {
    ...
    ExecuteWork(ctx, "SFDNSRES:GetAddrInfoRequest", std::move(work));
}
...
void SFDNSRES::GetHostByNameRequestWithOptions(Kernel::HLERequestContext& ctx) {
    ...
    ExecuteWork(ctx, "SFDNSRES:GetHostByNameRequestWithOptions", std::move(work));
}
...
void SFDNSRES::GetAddrInfoRequestWithOptions(Kernel::HLERequestContext& ctx) {
    ...
    ExecuteWork(ctx, "SFDNSRES:GetAddrInfoRequestWithOptions", std::move(work));
}
...
void SFDNSRES::ExecuteWork(Kernel::HLERequestContext& ctx, std::string sleep_reason, Work work) {
    ...
    ctx.SleepClientThread(std::move(sleep_reason), std::numeric_limits<u64>::max(),
...
SFDNSRES::SFDNSRES(Core::System& system_)
    : ServiceFramework("sfdnsres"), system{system_}, worker_pool{system, this, "SFDNSRES"} {
